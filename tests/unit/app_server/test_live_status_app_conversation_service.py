...
class TestModelNativeToolCallingReliable:
    ...
    @pytest.mark.parametrize(
        'model,llm_base_url,expected',
        [
            # Bare model name on Ollama default port - unreliable
            ('deepseek-r1:14b', 'http://localhost:11434', False),
            ('qwen2.5-coder:7b', 'http://192.168.1.50:11434', False),
            ('llama3.2:3b', 'http://host.docker.internal:11434', False),
            # Any model on port 11434 should be treated as Ollama
            ('gpt-4o', 'http://localhost:11434', False),
            # Same bare name on a different port - reliable (e.g. LM Studio)
            ('deepseek-r1:14b', 'http://localhost:1234', True),
            # No llm_base_url - cannot infer Ollama, assume reliable
            ('deepseek-r1:14b', None, True),
        ],
    )
    def test_model_native_tool_calling_reliable_with_base_url(
        self, model, llm_base_url, expected
    ):
        assert (
